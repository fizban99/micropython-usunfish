...
def reset_pos():
	...
	u.eg=0;u.last_mv=-1;u.ply=0;u.op_ind=_OP_IND;u.max_qs=_MAX_QS;u.position[:]=hist[-1][:];u.position[0]=hist[-1][0][:]
own_book=True
while True:
	...
	args=line.split()
	if args[0]=='uci':send('id name',version);send('id author',f"fizban99 ({year})");send(f"option name Skill Level type spin default {LEVEL} min 0 max 7");send('option name OwnBook type check default true');send(f"option name UCI_LimitStrength type check default {limit_strength}");send('uciok')
		...
		for mv in args[3:]:move_code=parse_move(mv,1-(u.position[2]>>20));u.mk_mv(move_code);u.history.append(u.ghash());u.history=u.history[-_MAX_HIST:];hist.append((u.position[0][:],u.position[1],u.position[2],u.position[3],u.position[4]))
	elif args[0]=='go'or args[0]=='bench':
		if u.ply==0:hist=[startpos];reset_pos()
		state=parse_go(args);start=monotonic();move_str=None;best_move=0;best_move_code=0;board,pscore,wc_bc_ep_kp,ksq,mob=hist[-1];turn='b'if wc_bc_ep_kp>>20 else'w';board=board[:]
		if args[0]=='bench':LEVEL=10;seed(0);u.HASH_XOR=0;u.op_mode=0;limit_strength=True;state[f"{turn}time"]=600000;state['infinite']=False
		...
		for(depth,gamma,score,mv)in u.search(gmv):
			if score>=gamma and mv:best_move=render_mv(mv,wc_bc_ep_kp>>20);best_move_code=mv;send('info depth',depth,'score cp',score,'nodes',u.nodes,'nps',u.nodes*1000//(monotonic()-start+1),'pv',best_move)
			if lvl==-1 and(best_move_code or u.nodes>125)or lvl>-1 and u.nodes>125*(1<<lvl)or score==_MT_LW and depth>=4 or u.max_time and monotonic()-max_time>0:break
