...
max_h_mv=[0,0]
eg=0
position=[[11,9,10,12,13,10,9,11,8,8,8,8,8,8,8,8,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,0,0,0,0,0,0,0,0,3,1,2,4,5,2,1,3],60|4<<8,1015936,0,0]
@micropython.native
def restore(mv,dif):
	pos=position;board,ksq,_,_,_=pos;board[mv>>8&255]=dif>>4&15;board[mv&63]=dif&15
...
@micropython.native
def ghash():board,ksq,wc_bc_ep_kp,pscore,_=position;h=bytes([(board[i]<<4)+1|board[i+1]for i in range(0,64,2)]);h=crc32(h)^HASH_XOR&1073741823^wc_bc_ep_kp;sign=wc_bc_ep_kp>>20;return-h if sign else h
@micropython.native
def reverse():
	pos=position;board,ksq,wc_bc_ep_kp,pscore,mob=pos
	for i in range(32):board[i],board[63-i]=board[63-i]^8,board[i]^8
	pos[1]=ksq>>8^63|(ksq&255^63)<<8
@micropython.native
def rotate_and_set(score,wc,bc,ep,kp,turn,nullmove,mob):pos=position;reverse();turn=turn^1;pos[3]=-score;pos[2]=turn<<20|bc<<18|wc<<16|(ep^63 if ep!=128 and not nullmove else 128)<<8|(kp^63 if kp!=128 and not nullmove else 128);pos[4]=-mob
@micropython.native
def rotate(nullmove=False):board,ksq,wc_bc_ep_kp,pscore,mob=position;turn=wc_bc_ep_kp>>20;wc=wc_bc_ep_kp>>18&3;bc=wc_bc_ep_kp>>16&3;ep=wc_bc_ep_kp>>8&255;kp=wc_bc_ep_kp&255;rotate_and_set(pscore,wc,bc,ep,kp,turn,nullmove,mob)
@micropython.native
def move(mv,val,position):
	pos=position;board,ksq,wc_bc_ep_kp,pscore,mob=pos;xor=(wc_bc_ep_kp>>20)*7;i,j,prom,turn=mv>>8,mv&63,((mv&255)>>6)+1,wc_bc_ep_kp>>20;xor=turn*7;p=board[i];wc,bc,ep,kp=wc_bc_ep_kp>>18&3,wc_bc_ep_kp>>16&3,wc_bc_ep_kp>>8&255,wc_bc_ep_kp&255;q=board[j];pp=p&7;t=pp if not eg or op_mode else PSTMAP[pp];val=value(pst,i,j,prom,p,q,xor,eg,kp,ep,t)if val is None else val;ep,kp=128,128;score=pscore+val;dif=board[i]<<4|board[j];board[j]=p;board[i]=6|turn<<3;wc=wc&1 if i==_A1 else wc&2 if i==_H1 else wc;bc=bc&2 if j==_A8 else bc&1 if j==_H8 else bc
	if p==_K:
		wc=0
		if abs(j-i)==2:kp=(i+j)//2;k=_A1 if j<i else _H1;dif=k<<16|kp<<8|dif;board[k]=6|turn<<3;board[kp]=_R
		ksq=ksq&65280|j
	elif p==_P:
		if _A8<=j<=_H8:board[j]=prom
		if j-i==2*_NO:ep=i+_NO
		if j==wc_bc_ep_kp>>8&255:board[j+_S]=6|turn<<3;dif=j+_S<<8|dif
	pos[1]=ksq;rotate_and_set(score,wc,bc,ep,kp,turn,False,mob);return dif
@micropython.native
...
def s_tp(h,mv,best,dr,val,od,fh,mob,incheck):
	...
	if fh:
		...
		if od>0:max_h_mv[turn]=s_hmv(h_mv[turn],h_va[turn],mv,max_h_mv[turn],-od*od)
	e=fh|best+16384|dr<<16|od+16<<20|iter<<25;it=iter;mv=mv|mob+512<<14|incheck>>1<<29;hind=h&3;new=False;tszs,tsch,tscd,md=t_szs[hind],tp_scoreh[hind],tp_scored[hind],max_d_sc[hind];i=get_index(h,tsch,0,tszs)
...
def g_sc(h,dr,od,board):
	global tp_scoreh,tp_scored;hind=h&3;tscd=tp_scored[hind]
	if dr>max_d_sc[hind]:return 0,_MT_UP,0,False,0
	i=get_index(h,tp_scoreh[hind],0,t_szs[hind])
	if i>=0:e=tscd[(i<<1)+1];tscd[(i<<1)+1]=e&33554431|iter<<25;mv=tscd[i<<1];position[4]=(mv>>14&1023)-512;incheck=mv>>29<<1;mv=mv&16383
	else:return 0,_MT_UP,0,False,0
	sod=(e>>20&31)-16
	if board[mv>>8]>5 or board[mv&63]<6 or board[mv>>8]==_P and mv>>8<mv&63:return 0,_MT_UP,0,False,0
	if sod!=od:return mv,-_MT_UP,32768,True,incheck
	fh=e&32768;best=(e&32767)-16384;return mv,best,fh,True,incheck
def reset_pos(omv,sc,lwc_bc_ep_kp,dif,omb):
	pos=position
	if not omv:return
	reverse();pos[3]=sc;pos[2]=lwc_bc_ep_kp;pos[4]=omb;restore(omv,dif)
@micropython.native
def bound(pos,g,od,cn,omv,val,gm,ind,gmv,incheck,lmr,gm_buf,req_d,max_time):
	global max_qs,nodes;board,ksq,wc_bc_ep_kp,sc,mob=pos;mqs=max_qs;osc=sc;omb=mob;lwc_bc_ep_kp=wc_bc_ep_kp
	if omv:dif=move(omv,val,pos);board,ksq,wc_bc_ep_kp,sc,mob=pos;q=board[omv&63]
	else:dif=None;q=6|wc_bc_ep_kp>>20<<3
	ret=0;best_mv=0;turn=wc_bc_ep_kp>>20
	while True:
		...
		entry=None;h=ghash();pdpth=req_d-od;hmove,e,fh,match,ret=g_sc(h,pdpth,od,board)
		if fh:
			if e>=g:ret,best,best_mv=1,e,hmove;break
		elif e<g:ret,best=1,e;break
		if match:mb=pos[4]-mob
		else:mb=0
		d=od if od>0 else 0
		if cn and d>0 and h in history:ret,best=1,0;break
		incheck=incheck>>1;incheck=incheck|2 if makes_check(ksq&255,8,pos,eg)else incheck
		if od<-mqs and not incheck:ret,best=1,sc+mb;break
		best=-_MT_UP;ret=0;break
	if not ret:
		while True:
			if not incheck&2 and d>2 and cn and abs(sc)<125:
				lwc=wc_bc_ep_kp;rotate(True);res=bound(pos,1-g,d-3,False,0,mb,gm,ind,gmv,incheck,0,gm_buf,req_d,max_time);res=-((res&65535)-16384);rotate();pos[2]=lwc;best=res if res>best else best
				if res>=g:best_mv=0;break
			if(incheck or req_d==1)and mqs<2*_MAX_QS:max_qs+=1
			...
			mb=pos[4]-mob;val=((gm[ind+l-1]&16777215)>>14)-512
			if not incheck and(q==14 or q==6)and d>0 and pdpth>2 and(sc+mb+val+_QS*d*5<g or sc+mb+val-_QS*d*5>=g):best=sc+mb+val;break
			ret=l
			while l:
				...
				if od<=-max_qs:
					...
					if not lmr and not incheck&2 and(ret-l>3 and pdpth>2):lmr=1
					...
					if best>=g:break
			break
		if best==-_MT_UP:best_mv=0;best=-_MT_LW if incheck==2 else 0
		if best>=g and(od>=-16 and best_mv!=0):s_tp(h,best_mv,best,pdpth,val,od,32768,pos[4],incheck)
		if best<g and not best_mv and hmove and od>=-16 and fh:s_tp(h,hmove,best,pdpth,val,od,0,pos[4],incheck)
		max_qs=mqs
...
def search(gmv):
	global nodes,req_d,tp_scored,tp_scoreh,max_d_sc,t_szs,op_ind,iter;global eg,max_qs,req_d,start_time;_,_,_,pscore,_=position;nodes=0
	if not gmv:gmv=g_mv()
	if op_mode==1:
	...
	for req_d in range(1,_MAX_DEPTH+1):
		...
		while eval_dist>_EVAL_ROUGHNESS+max(0,(req_d-4)*2):
			...
			eval_dist=upper-lower;yield(req_d,g,score,best_mv);g=(lower+upper+1)//2;iter+=1
history=list()
...
@micropython.native
def g_mv():
	global max_qs,eg,pst;global t_szs,max_d_sc;global max_h_mv;pos=position;lbrd,_,wc_bc_ep_kp,pscore,_=pos;turn=wc_bc_ep_kp>>20;pvalues=b'\x00\x03\x03\x05\t'
	...
	max_d_sc=[d,d,d,d];t_szs=ts;return gm
def recalc_tp(d,ts):
	_,_,wc_bc_ep_kp,pscore,mob=position;h=ghash();hind=h&3;tscd,tsch=tp_scored[hind],tp_scoreh[hind]
	try:i=tsch.index(h,ts[hind],t_szs[hind])
	except ValueError:return d
	e=tscd[(i<<1)+1];mv=tscd[i<<1]&16383;sod=(e>>20&31)-16
	if mv:j=ts[hind];tsch[j]=h;tscd[j<<1]=tscd[i<<1];tscd[(j<<1)+1]=e&4293984255|d<<16;ts[hind]+=1;lwc_bc_ep_kp=wc_bc_ep_kp;tsch[i]=0;dif=move(mv,0,position);d=recalc_tp(d+1,ts);reset_pos(mv,pscore,lwc_bc_ep_kp,dif,mob);return d
	else:return d
def can_kill_king(mv,ccheck=True):
	pos=position;lbrd,ksq,wc_bc_ep_kp,pscore,mob=pos;res=False;by_black=0;sc=pscore;lwc_bc_ep_kp=wc_bc_ep_kp
	...
	if mv>0:reset_pos(mv,sc,lwc_bc_ep_kp,dif,mob)
	return res
